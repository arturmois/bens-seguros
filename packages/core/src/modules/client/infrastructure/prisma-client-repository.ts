...
export class PrismaClientRepository implements ClientRepository {
  ...
  async create(data: CreateClientInput): Promise<ClientData> {
    const row = await this.prisma.client.create({
      data: {
        ...
        address:
          data.address === null || data.address === undefined ? Prisma.JsonNull : data.address,
        tags: data.tags ?? [],
  ...
  async findMany(filters: ClientFilters, page: CursorPage): Promise<Page<ClientData>> {
  ...
  async update(id: string, organizationId: string, data: UpdateClientInput): Promise<ClientData> {
    ...
    if (data.consentLgpd !== undefined) updateData.consentLgpd = data.consentLgpd;
    if (data.type !== undefined) updateData.type = data.type;
    if (data.address !== undefined) {
      updateData.address = data.address === null ? Prisma.JsonNull : data.address;
    }

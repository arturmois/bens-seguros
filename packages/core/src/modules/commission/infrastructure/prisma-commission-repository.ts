...
export class PrismaCommissionRepository implements CommissionRepository {
  ...
  async findById(id: string, organizationId: string): Promise<CommissionData | null> {
  ...
  async findMany(filters: CommissionFilters, page: CursorPage): Promise<Page<CommissionData>> {
    const where: Prisma.CommissionWhereInput = {
      ...
      ...(filters.search && {
        OR: [
          ...
          { policy: { policyNumber: { contains: filters.search, mode: 'insensitive' } } },
        ],
      }),
    };

...
export function commissionApprovedEmail(
  ...
  return baseLayout(`
    <h2 style="font-size:20px;font-weight:600;color:#0d4f4f;margin:0 0 16px;">Comissão Aprovada</h2>
    ...
    <p style="font-size:14px;color:#3f3f46;line-height:1.6;margin:0 0 16px;">
      Sua comissão referente à apólice <strong>${params.policyNumber}</strong> no valor de
      <strong>${params.value}</strong> foi aprovada por ${params.approvedBy}.
    </p>
    ${button('Ver Comissão', `${params.frontendUrl}/commissions`)}
  `)
}

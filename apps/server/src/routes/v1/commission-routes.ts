...
function handleCommissionError(error: unknown, reply: FastifyReply): FastifyReply {
...
export async function commissionRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        const commission = await useCase.execute(id, request.organizationId!, request.user!.id);
        return reply.send({ success: true, data: commission });
      } catch (error) {
        return handleCommissionError(error, reply);
      }

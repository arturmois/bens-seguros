...
export function useDocuments(entityType: DocumentEntityType, entityId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await api.get<DocumentData[]>(`/api/v1/documents?${params.toString()}`);
...
export function useUploadDocument() {
  ...
  return useMutation({
    mutationFn: async ({ entityType, entityId, file, type }: UploadInput) => {
      ...
      const res = await fetch(`${API_URL}/api/v1/documents/upload?${params.toString()}`, {
        method: 'POST',
        credentials: 'include',
        body: formData,
      });
...
export function useDocumentUrl(id: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<{ url: string }>(`/api/v1/documents/${id}/url`);
      return response.data;
    },
    staleTime: 60_000,
    enabled: id.length > 0,

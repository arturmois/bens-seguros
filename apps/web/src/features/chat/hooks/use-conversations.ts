...
export function useConversations(socket: Socket | null) {
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<ConversationListResponse> => {
      const response = await chatApi.get<ConversationData[]>(buildConversationsUrl(filters));
      return {
        data: response.data,
        meta: (response.meta ?? { total: 0, nextCursor: null }) satisfies ListMeta,
      };
    },
    staleTime: 60_000,
  });
  ...
  const handleIncomingMessage = useCallback(
    (payload: unknown) => {
      ...
      queryClient.setQueryData<ConversationListResponse>([CONVERSATIONS_KEY, filters], (prev) => {
        if (!prev) return prev;
  ...
  const assignConversation = useMutation({
    mutationFn: async (id: string) => {
      const response = await chatApi.post<ConversationData>(`/chat/conversations/${id}/assign`, {});
  ...
  const transferConversation = useMutation({
    mutationFn: async ({
      ...
      const response = await chatApi.post<ConversationData>(`/chat/conversations/${id}/transfer`, {
        toUserId,
        toUserName,
      });
      return response.data;
  ...
  const returnToQueue = useMutation({
    mutationFn: async (id: string) => {
      const response = await chatApi.post<ConversationData>(`/chat/conversations/${id}/return`, {});
      return response.data;
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: [CONVERSATIONS_KEY] });
  ...
  const closeConversation = useMutation({
    mutationFn: async (id: string) => {
      const response = await chatApi.post<ConversationData>(`/chat/conversations/${id}/close`, {});
      return response.data;
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: [CONVERSATIONS_KEY] });

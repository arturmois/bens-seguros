...
export function CommissionDetail({ commissionId }: CommissionDetailProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Separator />
      <CommissionActions commissionId={commissionId} currentStatus={data.status} />
    </div>
  );
}
...
function DetailHeader({ commission }: { readonly commission: CommissionData }) {
  return (
    <div className="flex flex-wrap items-center gap-3">
      <h2 className="text-xl font-semibold">{formatCurrency(commission.commissionValueInCents)}</h2>
...
function DetailInfoGrid({ commission }: { readonly commission: CommissionData }) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      />
      <InfoItem label="Paga em" value={commission.paidAt ? formatDate(commission.paidAt) : '-'} />
    </div>
  );
}

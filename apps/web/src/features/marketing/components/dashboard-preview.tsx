...
export function DashboardPreview(): React.ReactElement {
  return (
    <div className="mx-auto mt-16 max-w-4xl animate-float">
      <div className="rounded-2xl border border-white/8 bg-slate-900/80 p-5 shadow-[0_20px_60px_rgba(0,0,0,0.3)] backdrop-blur-xl supports-backdrop-filter:bg-white/3">
        <WindowChrome />
...
function StatCard({ label, value, change }: StatCardProps): React.ReactElement {
  return (
    <div className="rounded-xl border border-white/6 bg-white/4 p-4">
      ...
      <p className="mt-1 font-bold text-white text-xl">{value}</p>
...
function MiniBarChart(): React.ReactElement {
  return (
    <div className="mt-4 flex items-end gap-1.5 rounded-xl border border-white/6 bg-white/4 p-4">

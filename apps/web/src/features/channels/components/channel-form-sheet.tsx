      ...
      <SheetContent className="overflow-y-auto sm:max-w-lg">
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="mt-6 space-y-4 px-6">
          <FormField label="Nome" error={form.formState.errors.name?.message} required>
            <Input placeholder="Ex: WhatsApp Principal" {...form.register('name')} />
          </FormField>
          ...
          <FormField label="Numero" error={form.formState.errors.phoneNumber?.message}>
            <Input placeholder="Ex: +55 11 99999-0000" {...form.register('phoneNumber')} />
          </FormField>

          {watchedBrokerType === 'META' && (
            <>
              <FormField label="Token" error={form.formState.errors.metaToken?.message} required>
                <Input
                  type="password"
...
function FormField({ label, error, required, helperText, children }: FormFieldProps) {
  return (

...
export function createPairChannelProcessor(
  ...
  return async function processPairChannel(job: Job<PairChannelJobData>): Promise<void> {
    const { channelId, tenantId, phoneNumber } = job.data;

    logger.info({ channelId, tenantId, phoneNumber }, 'Processing pair-channel job');
    ...
    try {
      ...
      await pubsubRedis.publish(CHAT_PUBSUB_CHANNELS.PAIRING_CODE_RESULT, JSON.stringify(result));

      logger.info({ channelId, tenantId }, 'Pairing code generated successfully');
    } catch (err: unknown) {
      ...
      logger.error({ channelId, tenantId, err }, 'Failed to generate pairing code');
      throw err;
    }
  };
}

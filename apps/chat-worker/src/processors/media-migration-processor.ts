...
export async function processMediaMigration(_job: Job): Promise<void> {
  ...
  for (const msg of messages) {
    try {
      ...
      logger.error({ err, messageId: String(msg._id) }, 'Media migration failed for message');
      failed++;
    }
  }

  logger.info({ migrated, failed, total: messages.length }, 'Media migration batch completed');
}

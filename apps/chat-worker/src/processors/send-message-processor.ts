...
const PERMANENT_ERROR_CODES = ['INVALID_NUMBER', 'BLOCKED', 'BANNED', 'DEREGISTERED'] as const;
...
function selectBroker(
  ...
  if (brokerType === 'BAILEYS') {
    ...
    if (!broker) {
      throw new UnrecoverableError(`Baileys broker not found for channelId=${channelId}`);
    }
    return broker;
  }
...
export function createSendMessageProcessor(
  ...
  return async function processSendMessage(job: Job<SendMessageJobData>): Promise<void> {
    const { messageId, channelId, tenantId, to, text, mediaUrl, type } = job.data;
    ...
    if (result.status === 'FAILED') {
      ...
      if (isPermanentError(errorCode)) {
        ...
        logger.warn({ messageId, errorCode }, 'Permanent send failure, no retry');
    ...
    await pubsubClient.publish(
      ...
      JSON.stringify({ messageId, tenantId, status: 'SENT', externalId: result.externalId }),
    );

    logger.info({ messageId, externalId: result.externalId }, 'Message sent successfully');
  };
}

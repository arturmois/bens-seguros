...
function validateHmacSignature(rawBody: Buffer, signatureHeader: string): boolean {
...
export async function webhookRoutes(app: FastifyInstance): Promise<void> {
    ...
    async (
      ...
      if (!parsed.success) {
        return reply.status(400).send({
          success: false,
          error: { code: 'INVALID_QUERY', message: 'Missing required hub parameters' },
        });
      }
      ...
      if (mode !== 'subscribe') {
        return reply.status(400).send({
          success: false,
          error: { code: 'INVALID_MODE', message: 'hub.mode must be subscribe' },
        });
      }
      ...
      try {
        ...
        return reply.status(500).send({
          success: false,
          error: { code: 'MISCONFIGURED', message: 'Webhook verification not configured' },
        });
      }
  ...
  app.post('/chat/webhook/meta', async (request: FastifyRequest, reply: FastifyReply) => {
    const signatureHeader = request.headers['x-hub-signature-256'];

    if (typeof signatureHeader !== 'string') {
      return reply.status(401).send({
        success: false,
        error: { code: 'MISSING_SIGNATURE', message: 'X-Hub-Signature-256 header is required' },
      });
    }
    ...
    try {
      ...
      app.log.error('META_WHATSAPP_TOKEN is not configured for HMAC validation');
      return reply.status(500).send({
        success: false,
        error: { code: 'MISCONFIGURED', message: 'Webhook signature validation not configured' },
      });
    }
    ...
    for (const entryItem of entry) {
      for (const change of entryItem.changes) {
        app.log.debug({ accountId: entryItem.id, field: change.field }, 'Meta webhook change');

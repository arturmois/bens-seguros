...
export class MongooseConversationRepository implements ConversationRepository {
  async findById(id: string, tenantId: string): Promise<ConversationData | null> {
  ...
  async findMany(filters: ConversationFilters, page: CursorPage): Promise<Page<ConversationData>> {
    ...
    const [docs, total] = await Promise.all([
      Conversation.find(query).sort({ updatedAt: -1, _id: -1 }).limit(page.limit).lean(),
      Conversation.countDocuments({ tenantId: filters.tenantId, ...buildCountFilter(filters) }),
    ]);

    const items = (docs as unknown as MongooseConversationDoc[]).map(toConversationData);
  ...
  async create(data: ConversationData): Promise<ConversationData> {
    ...
    return toConversationData(doc.toObject() as unknown as MongooseConversationDoc);
  }
  ...
  async updateStatus(
    ...
    if (fields?.assignedTo !== undefined) updateFields['assignedTo'] = fields.assignedTo;
    if (fields?.assignedToName !== undefined)
      updateFields['assignedToName'] = fields.assignedToName;
    if (fields?.closedAt !== undefined) updateFields['closedAt'] = fields.closedAt;
    if (fields?.closedBy !== undefined) updateFields['closedBy'] = fields.closedBy;
  ...
  async atomicAssign(
    ...
    const doc = await Conversation.findOneAndUpdate(
      ...
      {
        $set: { status: 'HUMAN_ACTIVE', assignedTo: agentId, assignedToName: agentName },
      },
      { returnDocument: 'after' },
    ).lean();
  ...
  async findStaleConversations(olderThan: Date, limit: number): Promise<ConversationData[]> {
    ...
    return (docs as unknown as MongooseConversationDoc[]).map(toConversationData);
  }
}

function buildCountFilter(filters: ConversationFilters): Record<string, unknown> {
  ...
  if (filters.search) countFilter['lastMessageText'] = { $regex: filters.search, $options: 'i' };
  return countFilter;
}

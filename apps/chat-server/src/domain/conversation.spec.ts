...
function makeConversation(overrides: Partial<ConversationData> = {}): ConversationEntity {
...
describe('ConversationEntity', () => {
  ...
  describe('assign', () => {
    ...
    it('rejects assign on HUMAN_ACTIVE (already assigned)', () => {
      ...
      expect(() => conv.assign('agent-2', 'Maria')).toThrow(ConversationAlreadyAssignedError);
    });

    it('rejects assign on CLOSED', () => {
      const conv = makeConversation({ status: 'CLOSED' });

      expect(() => conv.assign('agent-1', 'João')).toThrow(InvalidConversationTransitionError);
    });
  ...
  describe('transfer', () => {
    ...
    it('rejects transfer from WAITING_HUMAN', () => {
      ...
      expect(() => conv.transfer('agent-2', 'Maria')).toThrow(InvalidConversationTransitionError);
    });

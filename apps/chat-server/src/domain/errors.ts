...
export const ChatErrors = {
  ...
  invalidTransition: (from: string, action: string): InvalidConversationTransitionError =>
  ...
  contactNotFound: (id: string): ContactNotFoundError => new ContactNotFoundError(id),
  channelNotFound: (id: string): ChannelNotFoundError => new ChannelNotFoundError(id),
};

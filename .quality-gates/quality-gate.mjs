...
function compare(current, baseline) {
  ...
  if (current.fileSize?.violationsCount != null && baseline.fileSize?.violationsCount != null) {
  ...
  if (current.duplication?.percentage != null && baseline.duplication?.percentage != null) {
...
function renderReport(current, baseline, comparison, mode) {
  ...
  if (mode === 'check' && !comparison.hasBaseline) {
    lines.push('- ⚠️  Nenhum baseline encontrado. Rode com `--baseline` pra congelar o estado atual.');
  }
  lines.push('');
  ...
  lines.push(`| Arquivos > ${fs.limit ?? 200} linhas | ${fs.violationsCount ?? 'n/a'} | ${fsBase.violationsCount ?? 'n/a'} |`);
  lines.push(`| Total de arquivos TS/TSX | ${fs.totalFiles ?? 'n/a'} | ${fsBase.totalFiles ?? 'n/a'} |`);

  const es = current.eslint || {};
  const esBase = baseline?.eslint || {};
  lines.push(`| ESLint errors | ${es.errors ?? 'n/a'} | ${esBase.errors ?? 'n/a'} |`);
  lines.push(`| ESLint warnings | ${es.warnings ?? 'n/a'} | ${esBase.warnings ?? 'n/a'} |`);

  const dup = current.duplication || {};
  const dupBase = baseline?.duplication || {};
  lines.push(`| Duplicação (%) | ${fmtPct(dup.percentage)} | ${fmtPct(dupBase.percentage)} |`);
  lines.push(`| Linhas duplicadas | ${dup.duplicatedLines ?? 'n/a'} | ${dupBase.duplicatedLines ?? 'n/a'} |`);

  const cov = current.coverage || {};
  const covBase = baseline?.coverage || {};
  if (cov.collected) {
    lines.push(`| Cobertura — linhas | ${fmtPct(cov.lines?.pct)} | ${fmtPct(covBase.lines?.pct)} |`);
    lines.push(`| Cobertura — branches | ${fmtPct(cov.branches?.pct)} | ${fmtPct(covBase.branches?.pct)} |`);
  } else {
...
async function main() {
  ...
  if (comparison.hasBaseline && comparison.regressions.length > 0) {
    console.log(`✗ ${comparison.regressions.length} regressão(ões) detectada(s)`);
    return 1;
  }
  return 0;
}
